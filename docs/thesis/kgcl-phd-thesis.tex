...
Experimental evaluation demonstrates that KGCL achieves sub-millisecond hook execution latency while maintaining semantic correctness guarantees. The Chicago School Test-Driven Development methodology ensures comprehensive validation through 251 tests: 140 unit tests with 0.24s total runtime, plus 111 integration tests using Testcontainers for real infrastructure validation against Oxigraph, Apache Fuseki, PostgreSQL, Redis, and RabbitMQ. The integration test suite validates all 43 WCP patterns with chaos engineering failure injection, multi-engine coordination with distributed locking, and Lean Six Sigma quality assurance (DMAIC workflows, FMEA analysis, SPC monitoring). The architecture successfully bridges the gap between symbolic AI (N3 reasoning) and practical workflow systems, providing a foundation for trustworthy, self-healing knowledge graph applications.

\textbf{Keywords:} Knowledge Graphs, Workflow Patterns, N3 Logic, Semantic Web, Self-Healing Systems, Reactive Architecture, YAWL, PyOxigraph, EYE Reasoner, Testcontainers, Chaos Engineering, Distributed Systems, Lean Six Sigma
...
\begin{itemize}
    ...
    \item \textbf{Chapter 8} describes the comprehensive testcontainers-based integration testing framework.
    \item \textbf{Chapter 9} discusses implications and future directions.
    \item \textbf{Chapter 10} concludes the dissertation.
\end{itemize}
...
% =============================================================================
% CHAPTER 8: INTEGRATION TESTING FRAMEWORK
% =============================================================================
\chapter{Integration Testing with Testcontainers}
\label{ch:integration}

This chapter presents a comprehensive integration testing framework using testcontainers to validate all 43 WCP patterns with real infrastructure components. The framework demonstrates that KGCL's semantic workflow patterns operate correctly across distributed services including RDF stores, message queues, and databases.

\section{Testcontainers Architecture}

\subsection{Infrastructure Components}

The integration testing framework employs five containerized services:

\begin{table}[h]
\centering
\caption{Testcontainer Infrastructure}
\label{tab:containers}
\begin{tabular}{llll}
\toprule
\textbf{Service} & \textbf{Image} & \textbf{Purpose} & \textbf{Port} \\
\midrule
Oxigraph & ghcr.io/oxigraph/oxigraph & RDF triple store & 7878 \\
Apache Fuseki & stain/jena-fuseki & Alternative RDF store & 3030 \\
PostgreSQL & postgres:16 & Audit trail, lockchain & 5432 \\
Redis & redis:7 & Runtime state, locks & 6379 \\
RabbitMQ & rabbitmq:3-management & Event coordination & 5672 \\
\bottomrule
\end{tabular}
\end{table}
...
Each container implements the \texttt{GenericContainer} protocol:

\begin{lstlisting}[language=Python,caption={Container Base Class}]
@dataclass
...
\section{WCP Pattern Validation Strategy}

\subsection{Pattern Coverage Matrix}

All 43 Van der Aalst Workflow Control Patterns are validated with real infrastructure:

\begin{table}[h]
\centering
\caption{WCP Pattern Test Coverage}
\label{tab:wcpcoverage}
\begin{tabular}{llrl}
\toprule
\textbf{Pattern Group} & \textbf{Patterns} & \textbf{Tests} & \textbf{Services} \\
\midrule
Basic Control Flow & WCP 1-5 & 12 & Oxigraph, PostgreSQL \\
Advanced Branching & WCP 6-11 & 18 & RabbitMQ, PostgreSQL \\
Multiple Instances & WCP 12-15 & 14 & PostgreSQL (parallel writes) \\
State-Based & WCP 16-18 & 9 & Redis (distributed locks) \\
Cancellation & WCP 19-27 & 24 & PostgreSQL (savepoints) \\
MI Joins & WCP 34-36 & 12 & Redis, PostgreSQL \\
Termination & WCP 43 & 6 & All services \\
\midrule
\textbf{Total} & \textbf{43 patterns} & \textbf{95} & --- \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Real-World Scenario Mapping}

Each pattern test implements a realistic business scenario:

\begin{itemize}
    \item \textbf{WCP-1 Sequence}: Insurance claim processing
    \item \textbf{WCP-2 Parallel Split}: Risk assessment routing
    \item \textbf{WCP-3 Synchronization}: Multi-department approval
    \item \textbf{WCP-6 Multi-Choice}: Compliance check routing
    \item \textbf{WCP-9 Discriminator}: First responder pattern
    \item \textbf{WCP-16 Deferred Choice}: External event routing
    \item \textbf{WCP-19 Cancel Task}: Order cancellation
    \item \textbf{WCP-35 Cancelling Partial Join}: Shipping quote selection
\end{itemize}

\section{Distributed Coordination Adapters}

\subsection{PostgreSQL Lockchain Writer}

The lockchain provides cryptographic provenance for workflow execution:

\begin{lstlisting}[language=Python,caption={Lockchain Writer}]
@dataclass(frozen=True)
class TickReceipt:
    workflow_id: str
    tick_number: int
...
class PostgresLockchainWriter:
    ...
    def write_tick_receipt(
        ...
        receipt_hash = self._compute_receipt_hash(
            workflow_id, tick_number, graph_hash,
            previous_hash, hook_results
        )
        # Insert into tick_receipts table
        return tick_id
...
\subsection{RabbitMQ Event Coordinator}

Event-driven coordination for workflow patterns:

\begin{lstlisting}[language=Python,caption={Event Coordinator}]
@dataclass(frozen=True)
class Event:
    event_type: str
...
class EventCoordinator:
    ...
    def track_correlation(self, correlation_id: str, event: Event)
...
\subsection{Redis Lock Manager}

Distributed locking for state-based patterns:

\begin{lstlisting}[language=Python,caption={Redis Lock Manager}]
class RedisLockManager:
    ...
    def acquire_exclusive(self, name: str, ttl: int) -> LockInfo
    def release_exclusive(self, name: str) -> bool
    def acquire_semaphore(self, name: str, max_count: int) -> str
    def create_barrier(self, name: str, count: int) -> str
    def wait_at_barrier(self, name: str, timeout: float) -> BarrierState
\end{lstlisting}

\section{Chaos Engineering Tests}

\subsection{Failure Mode Injection}

Six failure modes are systematically tested:

\begin{table}[h]
\centering
\caption{Chaos Engineering Failure Modes}
\label{tab:chaos}
\begin{tabular}{llll}
\toprule
\textbf{ID} & \textbf{Failure Mode} & \textbf{Injection} & \textbf{Expected Recovery} \\
\midrule
FM-CHAOS-001 & Container crash & Stop container & Reconnection \\
FM-CHAOS-002 & Network partition & Firewall rules & Timeout handling \\
FM-CHAOS-003 & Slow response & Artificial delay & Deadline enforcement \\
FM-CHAOS-004 & DB connection loss & Pool exhaustion & Connection retry \\
FM-CHAOS-005 & Queue unavailable & Channel close & Circuit breaker \\
FM-CHAOS-006 & Cascading failure & Multi-service & Graceful degradation \\
\bottomrule
\end{tabular}
\end{table}

\begin{lstlisting}[language=Python,caption={Chaos Test Example}]
def test_circuit_breaker_cascading_failure(
    self,
    postgres_connection: Any,
    redis_connection: Any,
    rabbitmq_channel: Any,
) -> None:
    """FM-CHAOS-006: Cascading failure with circuit breaker."""
    ...
    # Verify Redis fallback activated
    assert redis_connection.get("fallback:active") == b"true"
\end{lstlisting}

\section{Multi-Engine Coordination Tests}

\subsection{Race Condition Prevention}

Tests verify distributed locking prevents data races:

\begin{lstlisting}[language=Python,caption={Race Condition Test}]
def test_exclusive_lock_prevents_race_condition(
    self, redis_connection: Any
) -> None:
    """10 engines increment counter atomically."""
    manager = RedisLockManager(redis_connection)
    counter_key = "test:counter"
    redis_connection.set(counter_key, "0")
    num_engines = 10

    def increment_with_lock(engine_id: int) -> bool:
        with manager.exclusive_lock("counter_resource"):
            current = int(redis_connection.get(counter_key))
            time.sleep(0.01)  # Simulate processing
            redis_connection.set(counter_key, str(current + 1))
            return True

    with ThreadPoolExecutor(max_workers=num_engines) as executor:
        futures = [executor.submit(increment_with_lock, i)
                   for i in range(num_engines)]
        results = [f.result() for f in as_completed(futures)]

    # Without locking, lost updates would occur
    assert int(redis_connection.get(counter_key)) == num_engines
\end{lstlisting}

\subsection{Cross-Service Consistency}

Eventual consistency verified across all services:

\begin{lstlisting}[language=Python,caption={Cross-Service Consistency Test}]
def test_eventual_consistency_after_concurrent_updates(
    self,
    oxigraph_container: OxigraphContainer,
    postgres_connection: Any,
    redis_connection: Any,
) -> None:
    """Verify state consistency across RDF, PostgreSQL, Redis."""
    ...
    lock_manager = RedisLockManager(redis_connection)

    # Concurrent updates with lock coordination
    with ThreadPoolExecutor(max_workers=5) as executor:
        futures = [executor.submit(perform_update, i)
                   for i in range(5)]
        [f.result() for f in as_completed(futures)]

    # Assert: All services show consistent state
    rdf_updates = adapter.query("SELECT ?updates WHERE ...")
    redis_updates = int(redis_connection.hget(key, "updates"))
    pg_count = cursor.fetchone()[0]

    assert rdf_updates == redis_updates == pg_count == 5
\end{lstlisting}

\section{Lean Six Sigma Integration Tests}

\subsection{DMAIC Workflow Validation}

Complete DMAIC (Define, Measure, Analyze, Improve, Control) cycle tested:

\begin{table}[h]
\centering
\caption{DMAIC Phase Tests}
\label{tab:dmaic}
\begin{tabular}{llll}
\toprule
\textbf{Phase} & \textbf{Tests} & \textbf{Services} & \textbf{Artifacts} \\
\midrule
Define & 3 & PostgreSQL, Redis & Problem statement, CTQs \\
Measure & 4 & PostgreSQL & Baseline statistics \\
Analyze & 3 & Oxigraph, PostgreSQL & Root cause ontology \\
Improve & 4 & PostgreSQL, Redis & DOE results \\
Control & 5 & PostgreSQL, Redis & Control charts, alerts \\
\bottomrule
\end{tabular}
\end{table}

\begin{lstlisting}[language=Python,caption={DMAIC Analyze Phase Test}]
def test_analyze_root_cause_with_rdf_ontology(
    self, oxigraph_container, postgres_connection
) -> None:
    """Model fishbone diagram in RDF, query top causes."""
    adapter = RemoteStoreAdapter(...)

    # Load fishbone ontology
    adapter.load_turtle(fishbone_turtle)

    # Query causes by statistical correlation
    ...
    assert top_causes[0]["causeName"] == "Worn tooling"
    assert float(top_causes[0]["correlation"]) > 0.7
\end{lstlisting}

\subsection{FMEA Risk Prioritization}

Failure Mode and Effects Analysis with RPN calculation:

\begin{lstlisting}[language=Python,caption={FMEA RPN Calculation}]
def test_fmea_failure_modes_in_rdf(
    self, oxigraph_container, postgres_connection
) -> None:
    """Model failure modes, calculate RPN = S x O x D."""
    adapter = RemoteStoreAdapter(...)
    adapter.load_turtle(fmea_ontology)
    ...
    # Highest RPN: 8 * 6 * 4 = 192
    assert int(failure_modes[0]["rpn"]) == 192
\end{lstlisting}

\subsection{SPC Control Charts}

Statistical Process Control with Western Electric rules:

\begin{lstlisting}[language=Python,caption={SPC Western Electric Rules}]
def test_western_electric_rule_detection(
    self, postgres_connection, redis_connection
) -> None:
    """Detect Rule 4: 8 consecutive points below center."""
    # Data: 8 points below center line
    rule4_data = [9.98, 9.95, 9.92, 9.99, 9.94, 9.97, 9.93, 9.96]

    def check_rule4(points, center):
        consecutive_below = 0
        for p in points:
            if p < center:
                consecutive_below += 1
    ...
    assert check_rule4(rule4_data, center=10.0) is True
\end{lstlisting}

\section{End-to-End Business Scenarios}

\subsection{Order Fulfillment Scenario}

Complete order processing across all services:

\begin{enumerate}
    \item \textbf{Order Placement} (WCP-1): Sequence pattern
    \item \textbf{Compliance Checks} (WCP-2, WCP-3): Parallel split and sync
    \item \textbf{Payment Processing} (WCP-23): Retry loop pattern
    \item \textbf{Inventory Reservation} (WCP-16): Deferred choice with Redis lock
    \item \textbf{Shipping Quotes} (WCP-35): Cancelling partial join
    \item \textbf{Fulfillment} (WCP-5): Simple merge with notifications
\end{enumerate}

\begin{lstlisting}[language=Python,caption={E2E Order Fulfillment Test}]
def test_complete_order_fulfillment_happy_path(
    ...
    """Complete order workflow across all services."""
    adapter = RemoteStoreAdapter(...)
    lockchain = PostgresLockchainWriter(postgres_connection)

    # Phase 1-6: Execute workflow phases
    for phase in workflow_phases:
        phase.execute()
        lockchain.write_tick_receipt(...)

    # Assert final state across all services
    # 1. RDF state
    assert adapter.query(status_query) == "shipped"

    # 2. Lockchain integrity
    is_valid, _ = lockchain.verify_chain(workflow_id)
    assert is_valid

    # 3. Redis state
    assert redis_connection.hget(f"shipment:{order_id}", "tracking")
...
The integration tests run in GitHub Actions with service containers:
...
jobs:
  wcp-patterns:
    ...
    steps:
      - name: Start Oxigraph
        run: docker run -d --name oxigraph ...

      - name: Run WCP Pattern Tests
        run: uv run pytest tests/integration/patterns/ -v
\end{lstlisting}

\subsection{Test Execution Matrix}

\begin{table}[h]
\centering
\caption{CI/CD Test Jobs}
\label{tab:ci}
\begin{tabular}{lrrl}
\toprule
\textbf{Job} & \textbf{Tests} & \textbf{Timeout} & \textbf{Dependencies} \\
\midrule
Smoke Test & 12 & 10 min & PostgreSQL, Redis \\
RDF Stores & 24 & 20 min & Oxigraph \\
Message Queues & 18 & 20 min & RabbitMQ \\
Databases & 32 & 20 min & PostgreSQL, Redis \\
WCP Patterns & 95 & 45 min & All services \\
E2E Scenarios & 18 & 30 min & All services \\
Chaos Tests & 24 & 30 min & All services \\
Coordination & 28 & 20 min & All services \\
\midrule
\textbf{Total} & \textbf{251} & \textbf{195 min} & --- \\
\bottomrule
\end{tabular}
\end{table}
