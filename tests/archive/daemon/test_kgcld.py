...
class TestDaemonLimits:
    """Tests for DaemonLimits dataclass."""

    def test_default_limits(self) -> None:
        """Default limits have sensible values."""
        limits = DaemonLimits()
        assert limits.max_event_log_size == 100_000
        assert limits.max_snapshot_count == 10
        assert limits.query_timeout_seconds == 30.0
        assert limits.max_query_results == 10_000
        assert limits.max_template_size_bytes == 5 * 1024 * 1024
        assert limits.max_output_size_bytes == 50 * 1024 * 1024
        assert limits.max_bundle_iteration == 1_000
        assert limits.render_timeout_seconds == 60.0
        assert limits.n3_timeout_seconds == 30.0
        assert limits.n3_max_memory_mb == 512
    ...
    def test_custom_limits(self) -> None:
        ...
        assert limits.max_query_results == 5000
        assert limits.query_timeout_seconds == 15.0
        # Other limits remain default
        assert limits.max_event_log_size == 100_000

    def test_limits_are_frozen(self) -> None:
        """DaemonLimits is immutable."""
        limits = DaemonLimits()
        with pytest.raises(AttributeError):
            limits.max_query_results = 999  # type: ignore[misc]
...
class TestDaemonConfig:
    ...
    def test_default_limits_in_config(self) -> None:
        """Default config includes default limits."""
        config = DaemonConfig()
        assert isinstance(config.limits, DaemonLimits)
        assert config.limits.max_query_results == 10_000
    ...
    def test_custom_limits_in_config(self) -> None:
        """Can pass custom limits to config."""
        custom_limits = DaemonLimits(max_query_results=5000)
        config = DaemonConfig(limits=custom_limits)
        assert config.limits.max_query_results == 5000
...
# =============================================================================
# Service Gateway Integration Tests
# =============================================================================


class TestDaemonServiceGateway:
    """Tests for daemon service gateway integration."""

    def test_daemon_has_service_gateway(self, daemon: KGCLDaemon) -> None:
        """Daemon has service_gateway property."""
        from kgcl.daemon.service_gateway import ServiceGateway

        assert hasattr(daemon, "service_gateway")
        assert isinstance(daemon.service_gateway, ServiceGateway)

    def test_register_service(self, daemon: KGCLDaemon) -> None:
        """Can register external service."""
        from kgcl.daemon.service_gateway import ServiceReference
        ...
        assert "validator" in daemon.service_gateway.services
        assert daemon.service_gateway.services["validator"].uri == "http://localhost:8080/api/validate"

    def test_unregister_service(self, daemon: KGCLDaemon) -> None:
        """Can unregister external service."""
        from kgcl.daemon.service_gateway import ServiceReference

        ref = ServiceReference(service_id="temp", uri="http://localhost:9999")
        daemon.register_service(ref)
        daemon.unregister_service("temp")

        assert "temp" not in daemon.service_gateway.services

    def test_list_services(self, daemon: KGCLDaemon) -> None:
        """Can list registered services."""
        from kgcl.daemon.service_gateway import ServiceReference

        ref1 = ServiceReference(service_id="svc1", uri="http://a", assignable=True)
        ref2 = ServiceReference(service_id="svc2", uri="http://b", assignable=False)
        daemon.register_service(ref1)
        daemon.register_service(ref2)

        all_services = daemon.list_services()
        assignable = daemon.list_services(assignable_only=True)

        assert len(all_services) == 2
        assert len(assignable) == 1
        assert assignable[0].service_id == "svc1"

    @pytest.mark.asyncio
    async def test_delegate_task_requires_running(self, daemon: KGCLDaemon) -> None:
        """delegate_task requires daemon to be running."""
        from kgcl.daemon.service_gateway import ServiceReference

        ref = ServiceReference(service_id="test", uri="http://localhost:8080")
        daemon.register_service(ref)

        with pytest.raises(RuntimeError, match="Cannot delegate"):
            await daemon.delegate_task("test", "task-1", {})

    @pytest.mark.asyncio
    async def test_get_service_invocations_empty(self, running_daemon: KGCLDaemon) -> None:
        """get_service_invocations returns empty list initially."""
        invocations = running_daemon.get_service_invocations()
        assert invocations == []
