"""Projection CLI commands for template rendering.

This module provides Typer-based CLI commands for rendering templates
and bundles from the KGCL projection engine.

Examples
--------
$ kgcl proj render -t api.j2 -o api.py --param title=API
$ kgcl proj bundle -b crud.yaml -o output/ --dry-run
$ kgcl proj list
$ kgcl proj show api.j2
"""

from __future__ import annotations

import json
import sys
from pathlib import Path
from typing import Annotated, Any

import typer

from kgcl.projection.adapters.filesystem_registry import FilesystemTemplateRegistry
from kgcl.projection.domain.bundle import ConflictMode
from kgcl.projection.engine.bundle_renderer import BundleRenderer
from kgcl.projection.engine.projection_engine import ProjectionEngine
from kgcl.projection.ports.graph_client import GraphRegistry

__all__ = ["proj"]

proj = typer.Typer(help="Projection commands for template rendering", no_args_is_help=True)


def _parse_params(param_tuples: tuple[str, ...], params_file: Path | None) -> dict[str, Any]:
    """Parse parameters from CLI args and JSON file.

    Parameters
    ----------
    param_tuples : tuple[str, ...]
        Tuples of key=value strings.
    params_file : Path | None
        Path to JSON params file.

    Returns
    -------
    dict[str, Any]
        Merged parameters.

    Examples
    --------
    >>> _parse_params(("x=1", "y=hello"), None)
    {'x': '1', 'y': 'hello'}
    """
    params: dict[str, Any] = {}

    # Load from file if provided
    if params_file:
        with params_file.open() as f:
            params.update(json.load(f))

    # Parse CLI params (override file)
    for param_str in param_tuples:
        if "=" not in param_str:
            msg = f"Invalid param format (expected key=value): {param_str}"
            raise ValueError(msg)
        key, value = param_str.split("=", 1)
        params[key] = value

    return params


def _get_template_dir() -> Path:
    """Get default template directory.

    Returns
    -------
    Path
        Template directory path.
    """
    # Default to templates/ in current directory
    return Path.cwd() / "templates"


@proj.command()
def render(
    template: Annotated[str, typer.Option("-t", "--template", help="Template name")],
    out: Annotated[Path | None, typer.Option("-o", "--out", help="Output file path")] = None,
    param: Annotated[tuple[str, ...], typer.Option("-P", "--param", help="Parameter key=value")] = (),
    params: Annotated[Path | None, typer.Option("--params", exists=True, help="JSON params file")] = None,
    graph: Annotated[str, typer.Option("--graph", help="Graph ID to use")] = "main",
    template_dir: Annotated[Path | None, typer.Option("--template-dir", exists=True, help="Template directory")] = None,
) -> None:
    """Render a single template.

    Examples
    --------
    $ kgcl proj render -t api/service.j2 -o service.py
    $ kgcl proj render -t api.j2 -P title=API -P version=1.0
    $ kgcl proj render -t api.j2 --params config.json
    """
    try:
        # Parse parameters
        user_params = _parse_params(param, params)
        user_params["graph_id"] = graph

        # Set up registries
        tmpl_dir = template_dir if template_dir else _get_template_dir()
        template_registry = FilesystemTemplateRegistry(tmpl_dir)
        graph_registry = GraphRegistry()

        # Create engine and render
        engine = ProjectionEngine(template_registry, graph_registry)
        result = engine.render(template, user_params, graph)

        # Output
        if out:
            out.write_text(result.content, encoding="utf-8")
            typer.echo(f"Rendered to: {out}")
        else:
            typer.echo(result.content)

        # Show stats
        typer.echo(f"\n✓ Rendered in {result.render_time_ms:.2f}ms ({result.query_count} queries)", err=True)

    except Exception as e:
        typer.echo(f"Error: {e}", err=True)
        raise typer.Exit(code=1)


@proj.command()
def bundle(
    bundle_path: Annotated[str, typer.Option("-b", "--bundle", help="Bundle YAML path")],
    out_dir: Annotated[Path, typer.Option("-o", "--out-dir", help="Output directory")],
    param: Annotated[tuple[str, ...], typer.Option("-P", "--param", help="Parameter key=value")] = (),
    params: Annotated[Path | None, typer.Option("--params", exists=True, help="JSON params file")] = None,
    conflict: Annotated[str, typer.Option("--conflict", help="Conflict resolution mode")] = "error",
    dry_run: Annotated[bool, typer.Option("--dry-run", help="Don't write files")] = False,
    template_dir: Annotated[Path | None, typer.Option("--template-dir", exists=True, help="Template directory")] = None,
) -> None:
    """Render a bundle of templates.

    Examples
    --------
    $ kgcl proj bundle -b crud.yaml -o output/
    $ kgcl proj bundle -b api.yaml -o src/ --dry-run
    $ kgcl proj bundle -b crud.yaml -o out/ --conflict overwrite
    """
    try:
        # Validate conflict mode
        if conflict not in ("error", "overwrite", "skip"):
            typer.echo(f"Error: Invalid conflict mode: {conflict}. Must be one of: error, overwrite, skip", err=True)
            raise typer.Exit(code=1)

        # Parse parameters
        user_params = _parse_params(param, params)

        # Map conflict mode
        conflict_map = {"error": ConflictMode.ERROR, "overwrite": ConflictMode.OVERWRITE, "skip": ConflictMode.SKIP}
        conflict_mode = conflict_map[conflict]

        # Set up registries
        tmpl_dir = template_dir if template_dir else _get_template_dir()
        template_registry = FilesystemTemplateRegistry(tmpl_dir)
        graph_registry = GraphRegistry()

        # Load bundle
        bundle_registry = FilesystemTemplateRegistry(tmpl_dir)
        bundle_desc = bundle_registry.get_bundle(bundle_path)
        if bundle_desc is None:
            msg = f"Bundle not found: {bundle_path}"
            raise ValueError(msg)

        # Create renderer and render
        engine = ProjectionEngine(template_registry, graph_registry)
        renderer = BundleRenderer(engine)

        result = renderer.render_bundle(bundle_desc, user_params, out_dir, conflict_mode, dry_run)

        # Show results
        typer.echo(f"\n✓ Rendered {result.file_count} files:")
        for file_result in result.files:
            status = "[DRY RUN] " if dry_run else ""
            typer.echo(f"  {status}{file_result.output_path}")

        typer.echo(f"\nTotal time: {result.total_render_time_ms:.2f}ms", err=True)

    except Exception as e:
        typer.echo(f"Error: {e}", err=True)
        raise typer.Exit(code=1)


@proj.command("list")
def list_templates(
    template_dir: Annotated[Path | None, typer.Option("--dir", help="Template directory", exists=True)] = None,
) -> None:
    """List available templates.

    Examples
    --------
    $ kgcl proj list
    $ kgcl proj list --dir /path/to/templates
    """
    try:
        tmpl_dir = template_dir if template_dir else _get_template_dir()
        registry = FilesystemTemplateRegistry(tmpl_dir)

        templates = registry.list_templates()
        if not templates:
            typer.echo("No templates found.")
            return

        typer.echo(f"\nAvailable templates in {tmpl_dir}:")
        for tmpl in sorted(templates):
            typer.echo(f"  • {tmpl}")

        typer.echo(f"\nTotal: {len(templates)} templates")

    except Exception as e:
        typer.echo(f"Error: {e}", err=True)
        raise typer.Exit(code=1)


@proj.command()
def show(
    template_or_bundle: str = typer.Argument(..., help="Template or bundle name"),
    template_dir: Annotated[Path | None, typer.Option("--template-dir", exists=True, help="Template directory")] = None,
) -> None:
    """Show template or bundle details.

    Examples
    --------
    $ kgcl proj show api.j2
    $ kgcl proj show crud.yaml
    """
    try:
        tmpl_dir = template_dir if template_dir else _get_template_dir()
        registry = FilesystemTemplateRegistry(tmpl_dir)

        # Try as template first
        descriptor = registry.get(template_or_bundle)
        if descriptor:
            typer.echo(f"\nTemplate: {descriptor.id}")
            typer.echo(f"Engine: {descriptor.engine}")
            typer.echo(f"Language: {descriptor.language}")
            typer.echo(f"Framework: {descriptor.framework}")
            typer.echo(f"Version: {descriptor.version}")
            typer.echo(f"\nQueries: {len(descriptor.queries)}")
            for q in descriptor.queries:
                typer.echo(f"  • {q.name}: {q.purpose}")
            if descriptor.metadata.description:
                typer.echo(f"\nDescription:\n  {descriptor.metadata.description}")
            return

        # Try as bundle
        bundle_desc = registry.get_bundle(template_or_bundle)
        if bundle_desc:
            typer.echo(f"\nBundle: {bundle_desc.id}")
            typer.echo(f"Version: {bundle_desc.version}")
            typer.echo(f"Templates: {bundle_desc.template_count}")
            for entry in bundle_desc.templates:
                iter_info = f" (iterate: {entry.iterate.query})" if entry.iterate else ""
                typer.echo(f"  • {entry.template} → {entry.output}{iter_info}")
            if bundle_desc.description:
                typer.echo(f"\nDescription:\n  {bundle_desc.description}")
            return

        typer.echo(f"Not found: {template_or_bundle}")

    except Exception as e:
        typer.echo(f"Error: {e}", err=True)
        raise typer.Exit(code=1)
