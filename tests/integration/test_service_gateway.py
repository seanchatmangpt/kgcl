...
class TestServiceGatewayWithHTTPBin:
    ...
    @pytest.mark.asyncio
    async def test_invoke_post_endpoint_success(
        self, httpbin_container: dict[str, Any]
    ) -> None:
        """Verify successful POST to HTTPBin /post endpoint."""
        gateway = ServiceGateway()
        ref = ServiceReference(
            service_id="httpbin-post",
            uri=f"{httpbin_container['url']}/post",
            description="HTTPBin POST endpoint",
        )
        ...
        assert result.response_data.get("json") == {"test_data": "hello", "number": 42}
    ...
    async def test_invoke_with_custom_headers(
        ...
        ref = ServiceReference(
            ...
            uri=f"{httpbin_container['url']}/post",
            headers={"X-Custom-Header": "test-value", "X-Request-Id": "req-123"},
        )
        gateway.register(ref)
    ...
    @pytest.mark.asyncio
    async def test_invoke_404_not_found(
        self, httpbin_container: dict[str, Any]
    ) -> None:
        """Verify handling of 404 responses."""
        gateway = ServiceGateway()
        ref = ServiceReference(
            service_id="httpbin-404",
            uri=f"{httpbin_container['url']}/status/404",
        )
        gateway.register(ref)
    ...
    @pytest.mark.asyncio
    async def test_invoke_timeout_with_delay(
        self, httpbin_container: dict[str, Any]
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_invoke_multiple_services_parallel(
        self, httpbin_container: dict[str, Any]
    ) -> None:
        ...
        for i in range(3):
            ref = ServiceReference(
                service_id=f"httpbin-parallel-{i}",
                uri=f"{httpbin_container['url']}/post",
            )
            gateway.register(ref)
        ...
        tasks = [
            gateway.invoke(f"httpbin-parallel-{i}", f"task-p{i}", {"index": i})
            for i in range(3)
        ]
        ...
        for i, result in enumerate(results):
            assert result.response_data["json"]["index"] == i


class TestServiceGatewayWithMockAPI:
    """Integration tests using custom Mock API container."""

    @pytest.mark.asyncio
    async def test_invoke_task_endpoint(
        self, mock_api_container: dict[str, Any]
    ) -> None:
        """Verify task delegation to mock API."""
        gateway = ServiceGateway()
        ref = ServiceReference(
            service_id="mock-task",
            uri=mock_api_container["task_endpoint"],
            description="Mock task processor",
        )
        gateway.register(ref)
        ...
        assert result.response_data["task_id"] == "delegated-123"
        assert result.response_data["processed"] is True

    @pytest.mark.asyncio
    async def test_invoke_validate_endpoint(
        self, mock_api_container: dict[str, Any]
    ) -> None:
        """Verify validation service invocation."""
        gateway = ServiceGateway()
        ref = ServiceReference(
            service_id="mock-validate",
            uri=mock_api_container["validate_endpoint"],
        )
        gateway.register(ref)
        ...
        result = await gateway.invoke(
            ...
            payload={"document": "test document content"},
        )

        assert result.status == ServiceStatus.COMPLETED
        assert result.response_data["valid"] is True
        assert result.response_data["score"] == 0.95
        assert result.response_data["document"] == "test document content"

    @pytest.mark.asyncio
    async def test_invoke_slow_endpoint_within_timeout(
        self, mock_api_container: dict[str, Any]
    ) -> None:
        ...
        assert result.status == ServiceStatus.COMPLETED
        assert result.response_data["status"] == "delayed"
        assert result.duration_ms >= 3000  # At least 3 seconds

    @pytest.mark.asyncio
    async def test_invoke_slow_endpoint_timeout(
        self, mock_api_container: dict[str, Any]
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_invoke_error_endpoint(
        self, mock_api_container: dict[str, Any]
    ) -> None:
        """Verify handling of 500 Internal Server Error."""
        gateway = ServiceGateway()
        ref = ServiceReference(
            service_id="mock-error",
            uri=mock_api_container["error_endpoint"],
        )
        gateway.register(ref)
    ...
    @pytest.mark.asyncio
    async def test_invoke_echo_endpoint(
        self, mock_api_container: dict[str, Any]
    ) -> None:
        ...
        ref = ServiceReference(
            ...
            uri=mock_api_container["echo_endpoint"],
            headers={"X-Test-Header": "echo-test"},
        )
        gateway.register(ref)
        ...
        assert result.status == ServiceStatus.COMPLETED
        assert result.response_data["echo"]["message"] == "echo this back"
        assert result.response_data["echo"]["nested"]["key"] == "value"
        # Headers are also echoed
        assert "X-Test-Header" in result.response_data["headers"]

    @pytest.mark.asyncio
    async def test_invocation_history_tracking(
        self, mock_api_container: dict[str, Any]
    ) -> None:
        """Verify invocation history is properly tracked."""
        gateway = ServiceGateway()
        ref = ServiceReference(
            service_id="mock-history",
            uri=mock_api_container["task_endpoint"],
        )
        gateway.register(ref)
...
class TestServiceGatewayMultiContainer:
    ...
    async def test_delegate_to_multiple_backends(
        self,
        httpbin_container: dict[str, Any],
        mock_api_container: dict[str, Any],
    ) -> None:
        ...
        gateway.register(
            ServiceReference(
                service_id="backend-httpbin",
                uri=f"{httpbin_container['url']}/post",
            )
        )
        gateway.register(
            ServiceReference(
                service_id="backend-mock",
                uri=mock_api_container["task_endpoint"],
            )
        )
        ...
        assert httpbin_result.response_data["json"]["source"] == "httpbin"
        assert mock_result.response_data["task_id"] == "t1"
    ...
    async def test_mixed_success_and_failure(
        ...
        gateway.register(
            ServiceReference(
                service_id="success-service",
                uri=f"{httpbin_container['url']}/post",
            )
        )
        gateway.register(
            ServiceReference(
                service_id="failure-service",
                uri=mock_api_container["error_endpoint"],
            )
        )

        results = await asyncio.gather(
            gateway.invoke("success-service", "task-s", {}),
            gateway.invoke("failure-service", "task-f", {}),
        )
    ...
    async def test_service_discovery_pattern(
        ...
        registered = gateway.list_services()
...
class TestServiceGatewayRobustness:
    ...
    async def test_empty_payload(self, httpbin_container: dict[str, Any]) -> None:
        ...
        assert result.status == ServiceStatus.COMPLETED
        assert result.response_data["json"] == {}
    ...
    async def test_large_payload(self, httpbin_container: dict[str, Any]) -> None:
        ...
        assert result.status == ServiceStatus.COMPLETED
        assert len(result.response_data["json"]["data"]) == 100000
    ...
    async def test_special_characters_in_payload(
        ...
        assert result.status == ServiceStatus.COMPLETED
        assert result.response_data["json"]["unicode"] == "Hello 世界 🌍"
        assert "<script>" in result.response_data["json"]["html"]

    @pytest.mark.asyncio
    async def test_rapid_sequential_invocations(
        self, mock_api_container: dict[str, Any]
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_high_load(
        self, mock_api_container: dict[str, Any]
    ) -> None:
        ...
        tasks = [
            gateway.invoke("high-load", f"load-{i}", {"index": i}) for i in range(20)
        ]
        results = await asyncio.gather(*tasks)
