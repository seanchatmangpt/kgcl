...
class RDFConverter:
    ...
    def convert_event(self, event: AppEvent | BrowserVisit | CalendarBlock, graph: Graph | None = None) -> Graph:
    ...
    def _convert_app_event(self, event: AppEvent, graph: Graph) -> Graph:
        ...
        if event.duration_seconds is not None:
            self._add_property(graph, event_uri, "durationSeconds", event.duration_seconds, datatype=XSD.double)

        if event.process_id is not None:
            self._add_property(graph, event_uri, "processId", event.process_id, datatype=XSD.integer)
    ...
    def _add_property(
        self, graph: Graph, subject: URIRef, predicate: str, value: Any, datatype: URIRef | None = None
    ) -> None:
    ...
    def _add_timestamp(self, graph: Graph, subject: URIRef, predicate: str, timestamp: datetime) -> None:
