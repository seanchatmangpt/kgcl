#!/usr/bin/env python3
"""Generate view components from Java sources.

This script uses the unified code generation framework to convert
YAWL UI view Java classes to React components + FastAPI endpoints.
"""
...
def main() -> int:
    """Generate all view components."""
    # Input/output paths
    view_dir = Path("vendors/yawlui-v5.2/src/main/java/org/yawlfoundation/yawl/ui/view")
    output_dir = Path("src")  # Generator appends module path automatically
    template_dir = Path("src/kgcl/codegen/templates")

    # Find all Java files
    java_files = sorted(view_dir.glob("**/*.java"))
    print(f"Found {len(java_files)} Java files in view/")
