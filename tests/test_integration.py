...
class GraphChangeCondition(Condition):
    ...
    async def evaluate(self, context: dict[str, Any]) -> ConditionResult:
        ...
        return ConditionResult(
            triggered=has_changes,
            metadata={"triple_count": context.get("triple_count", 0)},
        )
...
class TestHookUnrdfIntegration:
    ...
    def test_hook_triggers_on_graph_change(self) -> None:
        ...
        engine.graph.add(
            (
                engine.namespace["entity1"],
                engine.namespace["hasValue"],
                engine.namespace["value1"],
            )
        )
    ...
    def test_threshold_hook_on_triple_count(self) -> None:
        ...
        for i in range(15):
            engine.graph.add(
                (
                    engine.namespace[f"entity{i}"],
                    engine.namespace["hasValue"],
                    engine.namespace[f"value{i}"],
                )
            )
        ...
        hook = Hook(
            ...
            condition=ThresholdCondition(
                field="triple_count", threshold=10, operator=ThresholdOperator.GT
            ),
            handler=graph_update_handler,
        )
...
class TestFullPipeline:
    ...
    @pytest.mark.asyncio
    async def test_ingest_trigger_hook_store_receipt(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            async def post_hook(events: list) -> None:
                pipeline = HookExecutionPipeline()
                context = {"triple_count": len(events)}
                for h in registry.get_all():
                    receipt = await pipeline.execute(h, context)
                    receipts.append(receipt)

            service.register_post_hook("executor", lambda events: None)
            service.start()
...
class TestPerformanceIntegration:
    ...
    def test_throughput_with_hooks(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            config = IngestionConfig(
                collector=CollectorConfig(output_directory=Path(tmpdir), batch_size=500)
            )
            service = IngestionService(config)
