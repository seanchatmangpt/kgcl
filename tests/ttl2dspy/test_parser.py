...
class TestPropertyShape:
    ...
    def test_is_required(self):
        ...
        prop = PropertyShape(
            path=URIRef("http://example.org/name"), name="name", min_count=1
        )
        assert prop.is_required is True

        prop2 = PropertyShape(
            path=URIRef("http://example.org/age"), name="age", min_count=0
        )
    ...
    def test_is_list(self):
        ...
        prop = PropertyShape(
            path=URIRef("http://example.org/tags"), name="tags", max_count=None
        )
        assert prop.is_list is True

        prop2 = PropertyShape(
            path=URIRef("http://example.org/name"), name="name", max_count=1
        )
...
class TestSHACLShape:
    ...
    def test_signature_name(self):
        ...
        shape = SHACLShape(
            uri=URIRef("http://example.org/PersonShape"), name="PersonShape"
        )
        assert shape.signature_name == "PersonSignature"

        shape2 = SHACLShape(
            uri=URIRef("http://example.org/TextAnalysis"), name="TextAnalysis"
        )
    ...
    def test_categorize_properties(self):
        ...
        shape = SHACLShape(
            ...
            properties=[
                ...
                PropertyShape(
                    path=URIRef("http://example.org/age"), name="age", default_value="0"
                ),
                PropertyShape(
                    path=URIRef("http://example.org/description"), name="description"
                ),
            ],
        )
