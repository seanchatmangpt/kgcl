...
class TestPropertyShape:
    ...
    def test_get_python_type_string(self):
        ...
        prop = PropertyShape(
            path=URIRef("http://example.org/name"), name="name", datatype=XSD.string, min_count=1, max_count=1
        )
    ...
    def test_get_python_type_integer(self):
        ...
        prop = PropertyShape(
            path=URIRef("http://example.org/age"), name="age", datatype=XSD.integer, min_count=0, max_count=1
        )
        assert prop.get_python_type() == "Optional[int]"

    def test_get_python_type_list(self):
        """Test Python type generation for lists."""
        prop = PropertyShape(path=URIRef("http://example.org/tags"), name="tags", datatype=XSD.string, max_count=None)
    ...
    def test_get_python_type_boolean(self):
        ...
        prop = PropertyShape(
            path=URIRef("http://example.org/active"), name="active", datatype=XSD.boolean, min_count=1, max_count=1
        )
