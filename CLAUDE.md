...
## 🚨 Implementation Lies Detection - Zero Tolerance

**MANDATORY:** Lies detector blocks commits, pushes, and pytest runs. No exceptions. No bypasses.

### Your Lie Tendencies (BLOCKED)

**NEVER write these patterns:**

1. **Deferred work:** `TODO`, `FIXME`, `XXX`, `HACK`, `WIP`, `STUB`, blanket `# noqa`, `# type: ignore`
2. **Stubs:** `pass`, `...`, `raise NotImplementedError`
3. **Empty returns:** `return {}`, `return None`, `return []` without logic
4. **Meaningless tests:** `assert True`, `assert result` (too vague), tests with no assertions
5. **Empty classes:** `class X: pass` or classes with only docstrings
6. **Temporal deferral:** Comments with "later", "for now", "temporary", "quick fix"
7. **Mocking domain objects:** `MagicMock()`, `Mock()`, `@patch`, custom `Mock*` classes

**ALWAYS:**
- Write complete implementations immediately
- Use `factory_boy` factories (from `tests.factories.*`) for test data
- Every test asserts specific behavior
- Remove unused imports/code immediately
- Complete functions fully - no placeholders

**Enforcement:** Pre-commit, pre-push, and pytest hooks scan automatically. Any lie = blocked.
