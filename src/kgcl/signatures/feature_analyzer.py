...
class FeatureAnalyzerInput(BaseModel):
    ...
    timestamps: list[str] = Field(
        ...
        description="ISO timestamps for each value (optional)"
    )
    context: str = Field(
        default="",
...
class FeatureAnalyzerOutput(BaseModel):
    ...
    outliers: list[dict[str, Any]] = Field(
        default_factory=list,
        description="Detected outliers with indices and values"
    )
    ...
    correlations: list[str] = Field(
        default_factory=list,
        description="Potential correlations or patterns"
    )
...
class FeatureAnalyzerModule:
    ...
    def _generate_recommendations(
        ...
        if trend == "increasing":
            ...
            recommendations.append(f"{feature_name} shows high volatility - consider establishing more consistent patterns")
    ...
    def _llm_analyze(self, input_data: FeatureAnalyzerInput) -> FeatureAnalyzerOutput:
        ...
        return FeatureAnalyzerOutput(
            trend=result.trend if result.trend in ["increasing", "decreasing", "stable", "volatile"] else trend_detected,
            outliers=outliers,
            summary_stats=stats,
