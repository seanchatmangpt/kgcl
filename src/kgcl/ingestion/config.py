...
class CollectorConfig(BaseModel):
    ...
    output_format: str = Field(default="jsonl", pattern="^(jsonl|json|parquet)$", description="Output file format")
    output_directory: Path = Field(default=Path.home() / ".kgcl" / "events", description="Directory for event logs")
...
class FilterConfig(BaseModel):
    ...
    excluded_apps: list[str] = Field(
        default_factory=lambda: ["com.apple.Spotlight", "com.apple.loginwindow", "com.apple.systemuiserver"],
        description="Application bundle IDs to exclude",
    )
    excluded_domains: list[str] = Field(
        default_factory=lambda: ["localhost", "127.0.0.1", "0.0.0.0"], description="Web domains to exclude"
    )
    min_duration_seconds: float = Field(default=1.0, ge=0, description="Minimum event duration to record")
    privacy_mode: bool = Field(default=False, description="Enable privacy mode (redact sensitive data)")
...
class FeatureConfig(BaseModel):
    ...
    enabled_features: list[str] = Field(
        default_factory=lambda: ["app_usage_time", "browser_domain_visits", "meeting_count", "context_switches"],
    ...
    incremental_updates: bool = Field(default=True, description="Enable incremental feature updates")
...
class RDFConfig(BaseModel):
    ...
    base_namespace: str = Field(default="http://kgcl.example.org/", description="Base RDF namespace URI")
    auto_namespace_assignment: bool = Field(default=True, description="Automatically assign namespaces to entities")
    ...
    include_schema_version: bool = Field(default=True, description="Include schema version in RDF output")
...
class ValidationConfig(BaseModel):
    ...
    shapes_directory: Path | None = Field(default=None, description="Directory containing SHACL shapes")
    strict_mode: bool = Field(default=False, description="Fail ingestion on validation errors")
    validation_report_format: str = Field(
        default="turtle", pattern="^(turtle|json-ld|nt|xml)$", description="Validation report format"
    )
...
class ServiceConfig(BaseModel):
    ...
    max_concurrent_requests: int = Field(default=10, ge=1, description="Maximum concurrent API requests")
