...
class DarkMatterOptimizer:
  ...
  def _apply_join_reordering(
    ...
    """
    join_indices = [
      i for i, s in enumerate(steps) if s.get("operation") == "join"
    ]
  ...
  def _apply_projection_pushdown(
    self, steps: List[Dict[str, Any]]
  ) -> Dict[str, Any]:
  ...
  def _find_parallelizable_steps(
    ...
    for i, step in enumerate(steps):
      ...
      for j in parallelizable:
        ...
        if step.get("step_id") in other_deps or steps[j].get(
          "step_id"
        ) in dependencies:
  ...
  def estimate_speedup(
    self, plan: Dict[str, Any], parallel_degree: int = 4
  ) -> float:
