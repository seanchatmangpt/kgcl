...
class TestFMCHAOS001ContainerCrash:
    ...
    def test_postgres_reconnect_after_failure(
        self,
        ...
        while retry_count < max_retries and not success:
            ...
            try:
                ...
                redis_connection.set(
                    f"{state_key}:tick",
                    json.dumps({"tick": retry_count, "timestamp": time.time()}),
                )
                success = True
            except Exception:
...
class TestFMCHAOS003TimeoutScenarios:
    ...
    def test_long_running_query_timeout(
        self,
        postgres_connection: Any,
    ) -> None:
...
class TestFMCHAOS005MessageQueueUnavailability:
    ...
    def test_message_queue_fallback(
        self,
    ...
    def test_cascading_failure_recovery(
        ...
        for i in range(5):
            ...
            if not check_circuit("redis"):
                try:
                    redis_connection.set(
                        f"cascade:{workflow_id}:op_{i}",
                        json.dumps({"op": i, "timestamp": time.time()}),
                    )
...
class TestRecoveryMechanisms:
    ...
    def test_retry_with_exponential_backoff(
        ...
        if success:
            redis_connection.set(
                f"retry:{workflow_id}:success",
                json.dumps({"attempts": attempt_count, "delays": delays}),
            )
    ...
    def test_orphan_state_cleanup(
        ...
        redis_connection.set(
            f"workflow:{active_workflow}:state",
            json.dumps({"status": "active", "last_heartbeat": time.time()}),
        )
