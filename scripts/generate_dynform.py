#!/usr/bin/env python3
"""Generate DynForm system from Java sources (26 files).

This script uses the unified code generation framework to convert
YAWL UI DynForm Java classes to React components + FastAPI endpoints.
"""
...
def main() -> int:
    """Generate all DynForm components."""
    # Input/output paths
    dynform_dir = Path("vendors/yawlui-v5.2/src/main/java/org/yawlfoundation/yawl/ui/dynform")
    output_dir = Path("src")  # Generator appends module path automatically
    template_dir = Path("src/kgcl/codegen/templates")

    # Find all Java files
    java_files = sorted(dynform_dir.glob("**/*.java"))
    print(f"Found {len(java_files)} Java files in DynForm")
